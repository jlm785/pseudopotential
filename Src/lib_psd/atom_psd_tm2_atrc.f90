...
subroutine atom_psd_tm2_atrc(ispp, jrc, r, arjrc, brjrc,                 &
...
! counter
  ...
  if (ispp == 'r') then
    arp = ka*arjrc/r(jrc) + (brjrc / (2*AI)) *                           &
                      (ev - vionic(jrc)/r(jrc) - vhxc(jrc) + 4*AI*AI)
  else
    arp = brjrc
  endif
  brc = arp / arc
  ...
  polydrc(2) = vrc - ev - 2*lp*polydrc(1)/rcpn(1)                        &
                        - polydrc(1)*polydrc(1)
...
end subroutine atom_psd_tm2_atrc
